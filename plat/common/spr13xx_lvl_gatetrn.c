...
void debug_swlvl_gatetrn(u32 slice);
#endif
...
#ifdef DEBUG_TRACK_DDR_INIT
	/* DEBUG */
	debug_swlvl_gatetrn(0);
	debug_swlvl_gatetrn(3);
#endif

	for (slice = 0; slice < DATA_SLICE_MAX; slice++) {
		if (slice_present[slice]==1)
		{
			gate_counter = 25; /* original was 7 */
			...
			if(slice == 3) {
  				uartDebug_Puts("\n\rbefore step_14\n\r");
				uartDebug_Puts("\n\r resp=");
				uartDebug_Putdec(resp);
				uartDebug_Puts("\n\r prelim_preamble_start=");
				uartDebug_Putdec(prelim_preamble_start);
				uartDebug_Puts("\n\r prelim_rise_edge=");
				uartDebug_Putdec(prelim_rise_edge);
				uartDebug_Puts("\n\r");
            }
#endif
...
void debug_swlvl_gatetrn(u32 slice) {
	...
	uartDebug_Puts("\n\r");
	uartDebug_Puts("slice =  ");
	uartDebug_Putdec(slice);
	uartDebug_Puts("\n\r");

	for (gate_counter = 0; gate_counter < 64; gate_counter++) {
		set_gate_parms(gate_counter, slice);
		resp = read_resp(slice);
		uartDebug_Puts("   gate_counter = ");
		uartDebug_Putdec(gate_counter);
		uartDebug_Puts("\tresp = ");
		uartDebug_Putdec(resp);
		uartDebug_Puts("\n\r");
	}
