...
{
	struct misc_regs *misc_p = (struct misc_regs *)CONFIG_SPEAR_MISCBASE;
	struct sysctrl_regs *scctrl_p =
		(struct sysctrl_regs *)CONFIG_SPEAR_SCBASE;

	/* Set system state to SLOW */
	writel(SLOW, &scctrl_p->scctrl);
	writel(PLL_TIM << 3, &scctrl_p->scpllctrl);
	...
	writel(NORMAL, &scctrl_p->scctrl);

	/* Wait for system to switch to normal mode */
	while (((readl(&scctrl_p->scctrl) >> MODE_SHIFT) & MODE_MASK)
		!= NORMAL);
}
...
#if defined(CONFIG_SPEAR320_MDI)
	/* enable clock of all peripherals */
	writel(readl(&misc_p->periph1_clken) | 0xFFFFFFF8,
			&misc_p->periph1_clken);
#endif
